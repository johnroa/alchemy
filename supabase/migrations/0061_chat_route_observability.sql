-- ============================================================================
-- 0061_chat_route_observability.sql
--
-- Extends pipeline observability with per-route chat latency segments so
-- request traces and admin rollups can attribute time spent in context load,
-- memory retrieval, and the primary chat LLM call.
-- ============================================================================
...
begin
  ...
  chat_route_stats as (
    select
      (e.event_payload->>'route')::text as route,
      count(*) as total_requests,
      count(*) filter (where coalesce((e.event_payload->>'cache_hit')::boolean, false)) as cache_hit_count,
      count(*) filter (where coalesce((e.event_payload->>'generation_reused_context')::boolean, false)) as generation_reused_count,
      count(*) filter (where e.safety_state = 'degraded') as degraded_count,
      round(avg(e.latency_ms)::numeric, 0) as avg_latency_ms,
      percentile_cont(0.5) within group (order by e.latency_ms) as p50_latency_ms,
      percentile_cont(0.95) within group (order by e.latency_ms) as p95_latency_ms,
      round(avg(nullif((e.event_payload->>'context_load_ms')::numeric, null))::numeric, 0) as avg_context_load_ms,
      round(avg(nullif((e.event_payload->>'memory_retrieval_ms')::numeric, null))::numeric, 0) as avg_memory_retrieval_ms,
      round(avg(nullif((e.event_payload->>'llm_ms')::numeric, null))::numeric, 0) as avg_llm_ms
    from public.events e
    where e.event_type = 'chat_route_timing'
      and e.created_at >= v_since
      and coalesce(e.event_payload->>'route', '') <> ''
    group by (e.event_payload->>'route')
  ),
  variant_health as (
    select
  ...
  select jsonb_build_object(
    ...
    'chat_routes', coalesce(
      (
        select jsonb_agg(
          jsonb_build_object(
            'route', crs.route,
            'total_requests', crs.total_requests,
            'degraded_count', crs.degraded_count,
            'cache_hit_count', crs.cache_hit_count,
            'generation_reused_count', crs.generation_reused_count,
            'avg_latency_ms', crs.avg_latency_ms,
            'p50_latency_ms', round(crs.p50_latency_ms::numeric, 0),
            'p95_latency_ms', round(crs.p95_latency_ms::numeric, 0),
            'avg_context_load_ms', coalesce(crs.avg_context_load_ms, 0),
            'avg_memory_retrieval_ms', coalesce(crs.avg_memory_retrieval_ms, 0),
            'avg_llm_ms', coalesce(crs.avg_llm_ms, 0)
          )
          order by crs.total_requests desc
        )
        from chat_route_stats crs
      ),
      '[]'::jsonb
    ),
    'variant_health', (
