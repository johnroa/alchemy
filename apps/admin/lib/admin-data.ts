...
export const getIngredientsData = async (): Promise<{
  ...
  const [
    ...
    client
      .from("recipe_ingredients")
      .select("ingredient_id,recipe_version_id,normalized_status,updated_at")
      .limit(50000),
    client
      .from("recipe_ingredients")
    ...
    client
      .from("ingredient_pair_stats")
      .select("ingredient_a_id,ingredient_b_id,updated_at")
      .gte("updated_at", previousWindowStartIso)
      .limit(20000)
  ]);
  ...
  if (ingredientMetricsResult.error && !isSchemaMissingError(ingredientMetricsResult.error)) {
    throw new Error(ingredientMetricsResult.error.message);
  }
  ...
  if (totalRecipeIngredientCountResult.error && !isSchemaMissingError(totalRecipeIngredientCountResult.error)) {
    throw new Error(totalRecipeIngredientCountResult.error.message);
  }
  if (unresolvedCountResult.error && !isSchemaMissingError(unresolvedCountResult.error)) {
    throw new Error(unresolvedCountResult.error.message);
  }
  if (recipesResult.error && !isSchemaMissingError(recipesResult.error)) {
    throw new Error(recipesResult.error.message);
  }
  if (aliasesWindowResult.error && !isSchemaMissingError(aliasesWindowResult.error)) {
    throw new Error(aliasesWindowResult.error.message);
  }
  if (ontologyAllResult.error && !isSchemaMissingError(ontologyAllResult.error)) {
    throw new Error(ontologyAllResult.error.message);
  }
  if (ontologyWindowResult.error && !isSchemaMissingError(ontologyWindowResult.error)) {
    throw new Error(ontologyWindowResult.error.message);
  }
  if (pairAllResult.error && !isSchemaMissingError(pairAllResult.error)) {
    throw new Error(pairAllResult.error.message);
  }
  if (pairWindowResult.error && !isSchemaMissingError(pairWindowResult.error)) {
    throw new Error(pairWindowResult.error.message);
  }
  ...
  const pairWindowRows = (pairWindowResult.data ?? []) as Array<{
  ...
  const enrichedIngredientCount = ingredientIdsAll.filter(
    (id) => (ingredientMetricById.get(id)?.confidence ?? null) != null
  ).length;
  ...
  for (const row of usageRows) {
    ...
    ingredientCountByVersionId.set(
      row.recipe_version_id,
      (ingredientCountByVersionId.get(row.recipe_version_id) ?? 0) + 1
    );
  ...
  const ingredientsPerRecipe = recipeCount > 0
    ...
    : null;
  const unresolvedPerRecipe = recipeCount > 0
    ? unresolvedRowsForCurrentRecipes / recipeCount
    : null;
  ...
  for (const row of pairWindowRows) {
    ...
    if (inWindow(row.updated_at, currentWindow.start, currentWindow.end)) {
      pairCurrentSet.add(key);
    }
    if (inWindow(row.updated_at, previousWindow.start, previousWindow.end)) {
      pairPreviousSet.add(key);
    }
  }
